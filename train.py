...
LastEditTime: 2021-05-17 15:20:17
'''
...
from en_data_utils import Tokenizer4Bert, MyDataset4Bert

from models import CNN, GCNN, DPCNN, RCNN, RNN_CNN
from models import LSTM, BILSTM, BILSTM_ATTENTION, BILSTM_SSA
from models import SIAMESE_CNN, SIAMESE_LSTM, SIAMESE_BILSTM_ATTENTION
from models import SIN, WIN, ESIM, MGIN
...
df_file path
'''
train_df = './data/mid_data/train.pkl'
dev_df = './data/mid_data/dev.pkl'
test_df = './data/mid_data/test.pkl'
...
class Instructor:
    def __init__(self, opt):
        ...
        if 'bert' in opt.model_name:
            ...
            self.trainset = MyDataset4Bert(train_df, tokenizer)
            self.devset = MyDataset4Bert(dev_df, tokenizer)
            self.testset = MyDataset4Bert(test_df, tokenizer)
        else:   
            ...
            embedding_matrix_N = build_embedding_matrix(
                word2idx=tokenizer.word2idx,
                embed_dim=300,
                format='word2vec',
                binary=True,
                pre_trained_file=PreEmbedding2,
                initial_embedding_matrix='{0}_{1}_matrix.dat'.format(str(300), 'GoogleNews'))
    ...
    def _reset_params(self):
        for child in self.model.children():
            if type(child) != BertModel: # skip pre-trained model params
...
def main(increase_seed):
    ...
    '''
    parser.add_argument('--model_name', default='mgin', type=str)
    ...
    '''
    parser.add_argument('--max_seq_len', default=100, type=int) # avg-65 0.9_cover_rate-200
    ...
    '''
    pre_trained_parameters
    '''
    parser.add_argument('--pretrained_model_dim', default=768, type=int)
    ...
    model_classes = {
        ...
        'siamese_bilstm_attention' : SIAMESE_BILSTM_ATTENTION,
        'sin' : SIN,
        'win' : WIN,
        'esim' : ESIM,
        'mgin' : MGIN,
    ...
    input_cols = {
        ...
        'bilstm_ssa': ['x_data'],
        'siamese_cnn' : ['x_body', 'x_punchline'],
        'siamese_lstm' : ['x_body', 'x_punchline'],
        'siamese_bilstm_attention' : ['x_body', 'x_punchline'],
        'sin' : ['x_data', 'x_body', 'x_punchline'],
        'win' : ['x_body', 'x_punchline'],
        'esim' : ['x_body', 'x_punchline'],
        'mgin' : ['x_body', 'x_punchline'],
        'bert_spc': ['x_data', 'att_mask', 'x_pair_data', 'pair_att_mask']
    }
