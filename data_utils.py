...
LastEditTime: 2021-05-18 15:49:59
'''
...
def build_tokenizer(max_seq_len, tokenizer_dat, data_paths, mode):
...
def build_embedding_matrix(word2idx, wv_path, wv_dim, wv_type, wv_initial_matrix):
    if os.path.exists(wv_initial_matrix):
        print("loading initial_matrix : ", wv_initial_matrix)
        embedding_matrix = pickle.load(open(wv_initial_matrix, "rb"))
    else:
        print("loading pre-trained embedding file : ", wv_path)
        embedding_matrix = np.zeros((len(word2idx) + 2, wv_dim))
        if wv_type == "glove":
            word_vec = _load_glove(wv_path, word2idx=word2idx)
        elif wv_type == "word2vec":
            word_vec = _load_word_vec(wv_path, binary=False, word2idx=word2idx)
        elif wv_type == "fasttext":
            word_vec = _load_fasttext(wv_path, word2idx=word2idx)
        print("build initial_embedding_matrix : ", wv_initial_matrix)
        for word, i in word2idx.items():
            vec = word_vec.get(word)
            if vec is not None:
                # 0行和len(word2idx)+1行是全0向量，没有对应预训练向量的词也用全0向量表示
                embedding_matrix[i] = vec
        pickle.dump(embedding_matrix, open(wv_initial_matrix, "wb"))
...
class MyDataset(Dataset):
    def __init__(self, df_name, tokenizer):
        ...
        all_data = []
        for i in range(0, len(df)):
            x_data = tokenizer.text_to_sequence(eval(df.words[i]))
            y_label = int(df.label[i])
            data = {
                'x_data' : x_data,
                'y_label': y_label
                }
